...
(function($, document, window, undefined) {
  ...
  var methods = (function() {
    ...
    var setUserObject = function(data, successCb) {
     ...
     // castingCookieObject = data.idUtente;
      setStorage('castingCookie', data.idUtente);
      successCb(data);
  ...
  Views.atomicform = async function() {
    try {
      await    methods.getYourProfile().then(function(data) {
        //console.log(data);
        if (!data.dati_personali) return;
  ...
  function checkCastingLogin() {
    ...
    var currentPage = currentPageComplete.split("/raicasting/");
